...
//
//   - `noosexit`: Этот анализатор запрещает прямые вызовы `os.Exit` в функции `main` пакета `main`.
//     Это поощряет возврат ошибок из `main` и обработку кодов выхода более контролируемым образом.
package main
